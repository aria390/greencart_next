...
const Products = () => {
  ...
  return (
    <div className="flex flex-col gap-6 px-32 py-20">
      ...
      <ul className=" gap-14 items-center grid grid-cols-4 ">
        {data?.products
          .filter((product) =>
          ...
          .map((item) => {
            ...
            return (
              <li
                key={item._id}
                className="flex flex-col group gap-2 p-6 border border-gray-300 rounded-lg"
              >
                <Link href={`/allProducts/${item.category}/${item._id}`}>
                  <div className="px-5 py-2">
                    <img
                      className="max-w-full group-hover:scale-105"
                      src={item.image[0]}
                      alt={item.name}
                    />
                  </div>
